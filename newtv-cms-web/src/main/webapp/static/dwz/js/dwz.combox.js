	...
	$.extend($.fn, {
		...
		combox:function(){
			...
			return this.each(function(i){
				...
				//是否为在远程获取数据
				var $ref = $(this);
				var remoteUrl = $ref.attr("remoteUrl");
				var defalutOption = $ref.attr("defalutOption");
				var flag = "true";
				if (defalutOption) {
					flag = defalutOption;
				}
				if(remoteUrl){
					$.ajax({
						async:false,
						type:'POST', dataType:"json", url:remoteUrl,cache: false,
						data:{},
						success: function(json){
							if (!json) return;
							...
							$.each(json, function(i){
								if (json[i] && json[i].length > 1){
									if($ref.attr("defaultVal")==json[i][0]){
										html += '<option selected value="'+json[i][0]+'">' + json[i][1] + '</option>';
									}else{
										html += '<option value="'+json[i][0]+'">' + json[i][1] + '</option>';
									}
								}
				...
				var selectDisabled = $this.attr("selectDisabled");
				var refUrl = $this.attr("refUrl") || "";

				var cid = $this.attr("id") || Math.round(Math.random()*10000000);
				var select = '';
				if(selectDisabled){
					select = '<div class="combox"><div id="combox_'+ cid +'" ' + (ref?' ref="' + ref + '"' : '') + '>';
				}else{
					select = '<div class="combox"><div id="combox_'+ cid +'" class="select"' + (ref?' ref="' + ref + '"' : '') + '>';
				...
				$("option", $this).each(function(){
					var option = $(this);
					options +="<li><a class=\""+ (value==option[0].value?"selected":"") +" bringComboxop_"+name+"_"+option[0].value+"\" href=\"#\" value=\"" + option[0].value + "\">" + option[0].text + "</a></li>";
				});
				options +="</ul>";
